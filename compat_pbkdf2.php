...
if (realpath($_SERVER['argv'][0]) === __FILE__ &&
  ...
  $password = 'password';
  $savedPassword = array('sha256','salt','65536',
    '4156f668bb31db3a17f4d1b91424ef0d417ad1f35d055aceaebd8da0f6a44b7e');
  list($algo, $salt, $iterations, $hash) = $savedPassword;
  var_dump(
    $hash,
...
if (!function_exists('hash_pbkdf2')) {
  function hash_pbkdf2($algo,
                       $password,
                       $salt,
                       $iterations,
                       $length = 0,
                       $rawOutput = false) {
    return compat_pbkdf2($algo,
                         $password,
                         $salt,
                         $iterations,
                         $length,
