...
if (!function_exists('hash_equals')) {
  ...
  function hash_equals($known_string, $user_string) {
    ...
    $user_string_type_error =
        'hash_equals(): Expected user_string to be a string, ' .
        gettype($user_string) . ' given'; // prepare wrong type error message
        // now to reduce the impact of string concatenation and the gettype call
    if (is_string($user_string) !== true) {
      trigger_error($user_string_type_error, E_USER_WARNING);
      // prevention of timing attacks might be still possible if we handle
      // $user_string as a string of different length (the trigger_error() call
    ...
    if ($known_string_len !== $user_string_len) {
      $res = $known_string ^ $known_string; // use $known_string instead of
      // $user_string to handle strings of diffrent length.
